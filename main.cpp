...
void display(){
	...
	if(help)
	{
		...
		glDisable(GL_LIGHTING);
		drawText("Taman Hiburan", 0, 0.55, 0.15);
		drawText("Selamat Datang di Taman Hiburan. \nGunakan tombol berikut untuk kontrol gerakan dan tindakan:", 0,0.4,0.82);
		drawText("* Panah Kiri dan Kanan: Melihat sekeliling 360 derajat", 0,0.30,0.82);
		drawText("* Panah Atas dan Bawah: Bergerak maju dan mundur", 0,0.25,0.82);
		drawText("* b : Mengganti latar belakang", 0,0.2,0.82);
		drawText("* c : ON/OFF Wahana Kora-Kora", 0,0.15,0.82);
		drawText("* h : Show/hide help menu", 0,0.1,0.82);
		drawText("* g : ON/OFF Wahana Bianglala", 0,0.05,0.82);
		drawText("* r : ON/OFF Wahana Roller Coaster", 0,0.0,0.82);
		drawText("* w : Ganti posisi kamera", 0,-0.05,0.82);
		drawText("* + : Bergeser naik", 0,-0.10,0.82);
		drawText("* - : Bergeser turun", 0,-0.15,0.82);
		drawText("* Seret mouse di layar ke arah kiri atau kanan: Melihat sekeliling 360 derajat", 0,-0.20,0.82);
		drawText("Tindakan di atas juga dapat dilakukan dari menu konteks klik kanan", 0,-0.25,0.82);
		drawText("Developed by Kelompok 11 Teknik Informatik 2018 R.2", 0,-0.45,0.82);
		glEnable(GL_LIGHTING);
		glPopMatrix();
	}
	else
...
void addMenu()
{
	...
	glutAddMenuEntry("Geser Bebas",0);
	glutAddMenuEntry("Naik Bianglala",1);
	glutAddMenuEntry("Naik Kora-Kora",2);
	glutAddMenuEntry("Naik Roller Coaster",3);
	submenu21 = glutCreateMenu(handle_gwheel_ring);
	glutAddMenuEntry("Kuning",0);
	glutAddMenuEntry("Silver",1);
	glutAddMenuEntry("Merah",2);
	glutAddMenuEntry("Cokelat",3);
	glutAddMenuEntry("Abu-abu",4);
	submenu22 = glutCreateMenu(handle_gwheel_trolley);
	glutAddMenuEntry("Biru",0);
	glutAddMenuEntry("Kuning",1);
	glutAddMenuEntry("Abu-abu",2);
	glutAddMenuEntry("Merah",3);
	glutAddMenuEntry("Cokelat",4);
	submenu2 = glutCreateMenu(handle_gwheel);
	glutAddMenuEntry("ON/OFF Bianglala",0);
	glutAddSubMenu("Warna Kerangka Bianglala",submenu21);
	glutAddSubMenu("Warna Sarang Bianglala",submenu22);
	submenu31 = glutCreateMenu(handle_columbus_body);
	glutAddMenuEntry("Cokelat Muda",0);
	glutAddMenuEntry("Hitam",1);
	glutAddMenuEntry("Abu-abu",2);
	glutAddMenuEntry("Cokelat Tua",3);
	glutAddMenuEntry("Ungu",4);
	submenu32 = glutCreateMenu(handle_columbus_stand);
	glutAddMenuEntry("Abu-abu Muda",0);
	glutAddMenuEntry("Putih",1);
	glutAddMenuEntry("Abu-abu Tua",2);
	glutAddMenuEntry("Cokelat",3);
	glutAddMenuEntry("Biru Tua",4);
	submenu3 = glutCreateMenu(handle_columbus);
	glutAddMenuEntry("ON/OFF Kora-Kora",0);
	glutAddSubMenu("Warna Perahu Kora-Kora",submenu31);
	glutAddSubMenu("Warna Penyangga Kora-Kora",submenu32);
	submenu41 = glutCreateMenu(handle_roller_color);
	glutAddMenuEntry("Silver",0);
	glutAddMenuEntry("Emas",1);
	glutAddMenuEntry("Abu-abu Tua",2);
	glutAddMenuEntry("Cokelat",3);
	glutAddMenuEntry("Ungu",4);
	submenu4 = glutCreateMenu(handle_roller);
	glutAddMenuEntry("ON/OFF Roller Coaster",0);
	glutAddSubMenu("Warna Roller Coaster",submenu41);
	glutCreateMenu(menu);
	glutAddSubMenu("Posisi Kamera",submenu1);
	glutAddSubMenu("Bianglala",submenu2);
	glutAddSubMenu("Kora-Kora",submenu3);
	glutAddSubMenu("Roller Coaster",submenu4);
	glutAddMenuEntry("Ganti Latar Belakang",0);
...
int main(int argc, char** argv)
{
		...
		glutCreateWindow("Taman Hiburan");
		initLights();
		initSky();
